...
public class AudioPlayer implements MethodCallHandler, Player.Listener, MetadataOutput {
    ...
    public AudioPlayer(
        ...
        if (audioOffloadPreferences != null) {
            this.audioOffloadPreferences = new AudioOffloadPreferences.Builder()
                    ...
                    .setIsSpeedChangeSupportRequired(
                            (Boolean) audioOffloadPreferences.get("isSpeedChangeSupportRequired"))
        ...
        if (audioLoadConfiguration != null) {
            ...
            Map<?, ?> livePlaybackSpeedControlMap = (Map<?, ?>) audioLoadConfiguration
                    .get("androidLivePlaybackSpeedControl");
            ...
            switch (call.method) {
                ...
                case "concatenatingInsertAll":
                    if (((String) call.argument("id")).length() == 0) {
                        ...
                        concatenating(call.argument("id"))
                                .addMediaSources(call.argument("index"), getAudioSources(call.argument("children")),
                ...
                case "concatenatingRemoveRange":
                    if (((String) call.argument("id")).length() == 0) {
                        ...
                        concatenating(call.argument("id"))
                                .removeMediaSourceRange(call.argument("startIndex"), call.argument("endIndex"), handler,
                                        () -> result.success(new HashMap<String, Object>()));
                        concatenating(call.argument("id"))
                                .setShuffleOrder(decodeShuffleOrder(call.argument("shuffleOrder")));
                    }
                ...
                case "concatenatingMove":
                    if (((String) call.argument("id")).length() == 0) {
                        ...
                        concatenating(call.argument("id"))
                                .moveMediaSource(call.argument("currentIndex"), call.argument("newIndex"), handler,
                                        () -> result.success(new HashMap<String, Object>()));
                        concatenating(call.argument("id"))
                                .setShuffleOrder(decodeShuffleOrder(call.argument("shuffleOrder")));
                    }
    ...
    private DefaultExtractorsFactory buildExtractorsFactory(Map<?, ?> options) {
        ...
        if (options != null) {
            ...
            if (androidExtractorOptions != null) {
                ...
                constantBitrateSeekingAlwaysEnabled = (Boolean) androidExtractorOptions
                        .get("constantBitrateSeekingAlwaysEnabled");
    ...
    private MediaSource decodeAudioSource(final Object json) {
        ...
        switch ((String) map.get("type")) {
            case "progressive":
                return new ProgressiveMediaSource.Factory(buildDataSourceFactory(mapGet(map, "headers")),
                        buildExtractorsFactory(mapGet(map, "options")))
    ...
    private AudioEffect decodeAudioEffect(final Object json, int audioSessionId) {
        ...
        switch (type) {
            case "AndroidLoudnessEnhancer":
                ...
                int targetGain = (int) Math.round((((Double) map.get("targetGain")) * 100.0)); // target gain needs to
                                                                                               // be provided in
                                                                                               // milliBel, the user
                                                                                               // provides the value in
                                                                                               // deciBel
    ...
    private void load(final List<MediaSource> mediaSources, ShuffleOrder shuffleOrder, final long initialPosition,
            final Integer initialIndex, final Result result) {
    ...
    private DefaultRenderersFactory buildDefaultRenderersFactoryWithPcmTee() {
        return new DefaultRenderersFactory(context) {
            ...
            protected AudioSink buildAudioSink(
                ...
                TeeAudioProcessor.AudioBufferSink pcmSink = new TeeAudioProcessor.AudioBufferSink() {
                    @Override
                    public void flush(
                            int sampleRateHz, int channelCount, @C.PcmEncoding int encoding) {
                        pcmRecorder.onFlush(sampleRateHz, channelCount, encoding);
                    }

                    @Override
                    public void handleBuffer(ByteBuffer buffer) {
                        pcmRecorder.onPcmBuffer(buffer);
                    }
                };
                TeeAudioProcessor tee = new TeeAudioProcessor(pcmSink);
                return new DefaultAudioSink.Builder(context)
                        .setEnableFloatOutput(enableFloatOutput)
                        .setEnableAudioTrackPlaybackParams(
                                enableAudioOutputPlaybackParameters)
                        .setAudioProcessors(new AudioProcessor[] { tee })
                        .build();
            }
        };
    }
    ...
    private void ensurePlayerInitialized() {
        if (player == null) {
            RenderersFactory renderersFactory = (eventHandler, videoListener, audioListener, textOutput,
                    metadataOutput) -> {
                Renderer[] defaultRenderers = buildDefaultRenderersFactoryWithPcmTee()
                        .createRenderers(
    ...
    private void loudnessEnhancerSetTargetGain(double targetGain) {
        int targetGainMillibels = (int) Math.round(targetGain * 100.0); // target gain needs to be provided in milliBel,
                                                                        // the user provides the value in deciBel
    ...
    private Map<String, Object> equalizerAudioEffectGetParameters() {
        ...
        for (short i = 0; i < equalizer.getNumberOfBands(); i++) {
            rawBands.add(mapOf(
                    "index", i,
                    "lowerFrequency", (double) equalizer.getBandFreqRange(i)[0] / 1000.0, // returns a value in
                                                                                          // milliHertz, we want Hertz
                    "upperFrequency", (double) equalizer.getBandFreqRange(i)[1] / 1000.0, // returns a value in
                                                                                          // milliHertz, we want Hertz
                    "centerFrequency", (double) equalizer.getCenterFreq(i) / 1000.0, // returns a value in milliHertz,
                                                                                     // we want Hertz
        ...
        return mapOf(
                "parameters", mapOf(
                        "minDecibels", equalizer.getBandLevelRange()[0] / 100.0, // returns a value in milliBel, we want
                                                                                 // deciBel
                        "maxDecibels", equalizer.getBandLevelRange()[1] / 100.0, // returns a value in milliBel, we want
                                                                                 // deciBel
                        "bands", rawBands));
    }
    ...
    private void equalizerBandSetGain(int bandIndex, double gain) {
        ((Equalizer) audioEffectsMap.get("AndroidEqualizer")).setBandLevel((short) bandIndex,
                (short) (Math.round(gain * 100.0))); // target gain needs to be provided in milliBel, the user provides
                                                     // the value in deciBel
    }
    ...
    /** Used from {@link MainMethodCallHandler} for single global method channel. */
    public void stopPcmRecording(Result result) {
        ensurePlayerInitialized();
        pcmRecorder.stopRecording(result);
    }
